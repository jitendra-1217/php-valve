...
abstract class TestCase extends \Jitendra\PhpValveTests\TestCase
{
    protected function makeAssertions(LeakyBucket\Base $limiter)
    {
        ...
        {
            $this->makeAssertionsPerAttempt($limiter, $resource, 1, 1, $limiterMaxBucketSize, $limiterMaxBucketSize - $i, millitime() + $limiterLeakFullTime);
        }
        // Subsequent attempts must fail withing current leak duration
        $this->makeAssertionsPerAttempt($limiter, $resource, 1, 0, $limiterMaxBucketSize, 0, millitime() + $limiterLeakFullTime);
        // Sleeping for while will leak and allow for new requests
        sleep($limiterLeakRateDuration/1000);
        $this->makeAssertionsPerAttempt($limiter, $resource, 1, 1, $limiterMaxBucketSize, $limiterLeakRateValue - 1, millitime() + $limiterLeakFullTime);
    }
}
